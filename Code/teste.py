...
def findBaseHomography():
    color_image1 = cv2.imread("{}/S1.jpg".format(QUARY_DIR))
...
def main():
    ...
    for i in range(2, 10):
        # Get next two images
        file_name = str(i)
        prior_image = cv2.imread("{}/S{}.jpg".format(QUARY_DIR, prior))
        current_image = cv2.imread("{}/S{}.jpg".format(QUARY_DIR, file_name))
